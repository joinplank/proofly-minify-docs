...
<AccordionGroup>
  ...
  <Accordion title="Use the SDK">
    The official SDK provides convenient methods for API requests and job status checking. You'll still need to implement polling yourself:
    
    ```typescript
    import { PlankProofly } from '@plank-proofly/api';
    
    const client = new PlankProofly({ apiKey: 'sk_...' });
    const { jobId } = await client.profiles.search({ name: 'John Doe' });
